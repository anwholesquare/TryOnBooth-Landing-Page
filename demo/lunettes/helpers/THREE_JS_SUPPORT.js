const TryOnHelper = (function() {
    ...
    function update_focals() {
        ...
        //console.log('INFO in TryOnHelper - focal_y =', fy);
    ...
    function init_three(maxFacesDetected) {
        // console.log('INFO in TryOnHelper - init_three(). Max faces detected = ', maxFacesDetected);
    ...
    function callbackReady(err, spec) {
        if (err) {
            // console.log('ERROR in TryOnHelper. ERR =', err);
        ...
        //console.log('INFO in TryOnHelper is ready. spec =', spec);
        ...
        // console.log('INFO in TryOnHelper: video resolution =', _videoElement.videoWidth, 'x', _videoElement.videoHeight);
        ...
        // init stabilizer:
        if (typeof(TryOnStabilizer) === 'undefined') {
            _stabilizers = null;
            // console.warn("WARNING in TryOnHelper: cannot find Stabilizer. Points won't be stabilized");
        } else {
            _stabilizers = [];
        }
        ...
        if (detectState.isDetected) {
            ...
            if (_stabilizers === null) {
                ...
                if (!_stabilizers[slotIndex]) {
                    _stabilizers[slotIndex] = TryOnStabilizer.instance(_spec.stabilizerSpec);
                };
    ...
    function compute_pose(landmarks, faceSlot) {
        ...
        const solved = TRYONBOOTHDRAW.compute_pose(objectPoints, imgPointsPx, _focals[0], _focals[1]);
    ...
    function start(domVideo) {
        ...
        TRYONBOOTHDRAW.init(_spec.spec);
    }
    ...
    const that = {
            ...
            if (_spec.videoURL) {
                ...
                return new Promise(function(accept, reject) {
                    domVideo.oncanplay = function(e) {
                        // console.log('INFO in TryOnHelper: video file can play');
        ...
        add_occluderFromFile: function(occluderURL, callback, threeLoadingManager, isDebug) {
            ...
            occluderMesh.scale.set(1.13, 1.13, 1.13);
            return occluderMesh;
        },
        ...
        change_NN: function(NNUrl) {
            return TRYONBOOTHDRAW.update({
                NNCPath: NNUrl
            }).then(function() {
                _landmarks.labels = TRYONBOOTHDRAW.get_LMLabels();
            });
        },


        reset_stabilizers: function() {
            // console.log('INFO in TryOnHelper: reset stabilizers')
        ...
        update_video: function(video) {
            return new Promise(function(accept, reject) {
                TRYONBOOTHDRAW.update_videoElement(video, function(newGlVideoTexture) {
