...
THREE.JeelizHelper = (function(){
  ...
  let _settings = {
    ...
    detectionThreshold: 0.3, // sensibility, between 0 and 1. Less -> more sensitive
    detectionHysteresis: 0.05,

    tweakMoveYRotateY: 0.5, // tweak value: move detection window along Y axis when rotate the face
    
    cameraMinVideoDimFov: 25, // Field of View for the smallest dimension of the video in degrees
  ...
  if (isMobile) { 
                ...
                _settings = {
						...
						cameraMinVideoDimFov: 72, // Field of View for the smallest dimension of the video in degrees
  ...
  function create_threeCompositeObjects(){
    for (let i=0; i<_maxFaces; ++i){
      ...
      if (_settings.isDebugPivotPoint){
        ...
        console.log('DEBUG in Helper: set the position of <pivot> in the console and report the value into Helper.js for _settings.pivotOffsetYZ');
      }
    }
  }
  ...
  const that = {
    ...
    update_camera: function(threeCamera){
      ...
      threeCamera.fov = fov;
      console.log('INFO in Helper.update_camera(): camera vertical estimated FoV is', fov, 'deg');
