const TryOnBooth = (function() {
    ...
    function tweak_faceAccessory(threeFaceAccessory) {
        if (_spec.isGlasses) {
            ...
            threeFaceAccessory.scale.multiplyScalar(82);
            ...
            // +Y -> move up
            threeFaceAccessory.position.set(0, 40, 63);
    ...
    function load_faceAccessory(modelURL, callback) {
        ...
        const scene = TryOnHelper.getThree().scene;
        scene.children.forEach(function(e) {

            if (e.type === "DirectionalLight") {
        ...
        if (modelURL === "assets/models3D/c2.glb") {
            // console.log("ss");
            light = new THREE.DirectionalLight(0xcaecfa, 7);
            light.position.set(1000, 200, 100);
            light.rotation.set(0, -45, 0);
            light.castShadow = false;
            scene.add(light);


            light2 = new THREE.DirectionalLight(0xcaecfa, 7);
            light2.position.set(-300, 200, 100);
            light2.rotation.set(0, 45, 0);
            light2.castShadow = false;
            scene.add(light2);

            light3 = new THREE.DirectionalLight(0xf6feff, 7.5);
            light3.position.set(50, 200, 300);
            ...
            model.scene.traverse(function(node) {
                if (node.isMesh) {
                    ...
                    if (node instanceof THREE.Mesh) {
                        ...
                        if (node.name === "lens" && modelURL === "assets/models3D/c2.glb") {
    ...
    function build_scene() {
        ...
        if (_spec.modelURL == "assets/models3D/c2.glb") {
            // light = new THREE.DirectionalLight(0xcaecfa, 7);
            // light.position.set(1000, 200, 100);
            // light.rotation.set(0, -45, 0);
            // light.castShadow = false;
        ...
        if (_spec.occluderURL) {
            TryOnHelper.add_occluderFromFile(_spec.occluderURL, null, _threeInstances.loadingManager, _spec.debugOccluder);
        }
        ...
        _threeInstances.loadingManager.onLoad = function() {
            //console.log('INFO in TRYONBOOTH: fully loaded');
            if (_spec.callback) {
    ...
    const that = {
        init: function(spec) {
            return new Promise(function(resolve, reject) {
                ...
                if (spec.solvePnPObjPointsPositions) {
                    TryOnFeatures.solvePnPObjPointsPositions = spec.solvePnPObjPointsPositions;
                }
                if (spec.solvePnPImgPointsLabels) {
                    TryOnFeatures.solvePnPImgPointsLabels = spec.solvePnPImgPointsLabels;
                }
                TryOnHelper.init(TryOnFeatures);
        ...
        pos: function() {
            return _threeInstances.faceAccessory;
        },
        ...
        pause: function(isStopVideoStream) {
            ...
            TRYONBOOTHDRAW.toggle_pause(true, isStopVideoStream);
        ...
        resume: function(isStopVideoStream) {
            ...
            TRYONBOOTHDRAW.toggle_pause(false, isStopVideoStream);
            _state = _states.idle;
            return true;
        },
        ...
        capture_image: function(callback) {
            ...
            const cvBg = TRYONBOOTHDRAW.capture_image(true);
            const width = cvBg.width,
                height = cvBg.height;
        ...
        resize: function(width, height) {
            ...
            _timerResize = setTimeout(function() {
                ...
                TryOnHelper.resize(width * s, height * s);
                _timerResize = null;
            }, _spec.resizeDelay);
            return true;
        },
        ...
        destroy: function() {
            return new Promise(function(accept, reject) {
                TRYONBOOTHDRAW.destroy().finally(function() {
